...
# ---- 0. безопасное чтение секретов из состояния ----------------------------
# НЕ source (мусорный/старый файл выполнил бы строки как команды) — берём только
# нужные ключи через grep. Любые посторонние строки в файле просто игнорируются.
load_secret() { [ -f "$STATE_FILE" ] && grep -h "^$1=" "$STATE_FILE" 2>/dev/null | head -1 | cut -d= -f2- || true; }
...
# ---- 2. секреты: из состояния (если есть) либо генерируем -------------------
XHTTP_SECRET="${XHTTP_SECRET:-$(load_secret XHTTP_SECRET)}"; XHTTP_SECRET="${XHTTP_SECRET:-$(openssl rand -hex 32)}"
SID1="${SID1:-$(load_secret SID1)}"; SID1="${SID1:-$(openssl rand -hex 1)}"
SID2="${SID2:-$(load_secret SID2)}"; SID2="${SID2:-$(openssl rand -hex 2)}"
SID4="${SID4:-$(load_secret SID4)}"; SID4="${SID4:-$(openssl rand -hex 4)}"
SID8="${SID8:-$(load_secret SID8)}"; SID8="${SID8:-$(openssl rand -hex 8)}"
...
REALITY_PRIV="${REALITY_PRIV:-$(load_secret REALITY_PRIV)}"
REALITY_PUB="${REALITY_PUB:-$(load_secret REALITY_PUB)}"
