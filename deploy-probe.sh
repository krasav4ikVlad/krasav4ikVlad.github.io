...
#
# ВАЖНО: смысл зонда — мерить через РЕАЛЬНО ЗАБЛОКИРОВАННЫЙ канал (как видит
# конечный пользователь за ТСПУ / у сотового оператора). Подходит любая машина,
# с которой заблокированные сервисы реально недоступны — это может быть домашний
# интернет ИЛИ VPS, стоящий за фильтром РКН. На «чистом» датацентре, где всё
# открывается напрямую, зонд бесполезен (покажет «всё работает»). На машине НЕ
# должен быть включён VPN/прокси.
#
...
# ---- точка измерения: заблокирован ли канал (тогда зонд осмыслен) -----------
# критерий не «VPS или дом», а «реально ли отсюда недоступны заблокированные
# сервисы». Проверяем Telegram: недоступен -> канал под фильтром, точка годная.
if command -v curl >/dev/null 2>&1; then
  hosting="$(curl -fsS --max-time 8 'http://ip-api.com/line/?fields=hosting' 2>/dev/null || true)"
  tg=1; curl -fsS --max-time 6 -o /dev/null 'https://web.telegram.org/' 2>/dev/null || tg=0
  if [ "$tg" = "0" ]; then
    log "Telegram с этой машины недоступен — канал под фильтром (ТСПУ). Отличная точка для зонда."
  elif [ "$hosting" = "true" ]; then
    warn "Похоже на ЧИСТЫЙ датацентр (Telegram открывается) — замеры будут «как из ДЦ», не как у пользователя."
  fi
fi
