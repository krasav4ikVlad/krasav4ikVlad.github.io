...
## Процесс не стартует, а логи пустые

Пустой лог приложения — это не «нечего сказать», а «до приложения дело не
дошло»: pm2 не смог его запустить. Счётчик `↺` при этом растёт, статус
уходит в `stopped`, и `pm2 logs` молчит, потому что писать было некому.

Смотреть надо не в лог приложения, а в лог самого pm2:

```bash
tail -40 ~/.pm2/pm2.log
```

Две причины, дающие ровно такую картину:

```bash
ls /home/rsvpn-bot/logs                  # нет каталога → pm2 падает с ENOENT
ls /home/rsvpn-bot/.venv/bin/python      # нет питона  → нечего запускать
```

Каталог `logs` создаёт сам `ecosystem.config.js`, но старые копии конфига
этого не умели. Лечится одной командой: `mkdir -p /home/rsvpn-bot/logs`.

Если pm2 отвечает `Process N not found` на `pm2 start <имя>` — его запись о
процессе испорчена. Помогает пересоздать её:

```bash
pm2 delete rsvpn-bot rsvpn-api
cd /home/rsvpn-bot && pm2 start ecosystem.config.js
pm2 save
```
