...
class RenewalService:
    ...
    async def renew(self, user: dict, expires) -> str:
        ...
        if not plan:
            # Срок, которого нет среди тарифов. Так выглядели все, кто пришёл
            # через бесплатный период: там в vpn.period записывалась его длина
            # (3 дня), тарифа на 3 дня нет — и человек просто никогда не
            # продлевался, при живом балансе и без единой ошибки в логе.
            # Чиним на месте: ставим самый короткий тариф и продлеваем по нему.
            plan = await self.plans.shortest()
            if not plan:
                return 'unknown_plan'
            await self.users.set_vpn(user_id, {'period': int(plan['days'])})
            log.warning('у %s был срок %s без тарифа — переведён на «%s»',
                        user_id, vpn.get('period'), plan['title'])
        ...
        # Скидка аудитории по умолчанию действует и на автопродление: иначе
        # цена на экране и цена в списании разошлись бы ровно на размер акции.
        # Но акция «вернись со скидкой» — это приглашение вернуться, а не
        # новый прайс, поэтому её можно оставить только на ручное действие:
        # выключатель «Скидка действует и на автопродление» в настройках.
        with_discount = (self.discounts is not None
                         and await self.settings.flag('discount.on_autorenew'))
        price = (await self.discounts.price(user, plan) if with_discount
                 else int(plan['price']))
