...
def length(text: str) -> int:
    ...
    Считать по самой разметке нельзя ни в каком виде. Жирный, ссылки и
    особенно кастомные эмодзи живут отдельно от текста — один такой значок
    в HTML занимает под полсотни знаков (`<tg-emoji emoji-id="…">🎟</tg-emoji>`),
    а для Telegram это один символ. Пост из восьми значков и семисот букв
    по разметке «не влезает в 1024», хотя на самом деле влезает вдвое.

    Эмодзи при этом считается за две единицы, а не за одну, — так их
    считает и сам Telegram, поэтому предел совпадает с его ответом.
    """
...
def preview_problem(text: str, photos=()) -> str:
    ...
    На вход идёт текст без разметки — тот, что видит человек. Отказ
    называет, на сколько знаков не влезло, и что с этим делать: «длиннее
    1024» без числа и без выхода — это тупик, а пост уже написан.
    """
    photos = list(photos or [])
    size = length(text)

    if photos:
        if size <= CAPTION_LIMIT:
            return ''
        return (f'Подпись к картинке у Telegram не длиннее '
                f'{CAPTION_LIMIT} знаков, а здесь {size} — лишних '
                f'{size - CAPTION_LIMIT}.\n\nВыходы: сократить на '
                f'{size - CAPTION_LIMIT} знаков; или отправить пост без '
