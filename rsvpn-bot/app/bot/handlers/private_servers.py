...
async def choose_location(call: types.CallbackQuery, callback_data: Server, c,
    ...
    # Пока в продаже одни безлимитные, объяснять разницу между ними и
    # лимитными незачем: человек её не увидит. Появится лимитная — подпись
    # вернётся сама.
    if all(not loc.limited for loc in places):
        hint = ('Все площадки — без ограничения по трафику: сколько скачаете, '
                'столько и скачаете.')
    elif plan.shared:
        hint = ('Все площадки под этот тариф — без ограничения по трафику: на '
                'машине живут три покупателя со своими людьми, и терабайт на '
                'всех кончился бы за месяц.')
    else:
        hint = ('Площадки с пометкой «1 ТБ» ограничены по трафику на весь '
                'сервер в месяц — на несколько человек этого хватает с '
                'запасом. Отмеченные «безлимит» работают без ограничения '
                'по трафику.')
