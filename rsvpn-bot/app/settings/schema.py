...
SCHEMA: tuple[Group, ...] = (
    ...
    Group('privacy', f'{e("lock")} Показ идентификаторов', (
        Setting('privacy.mask_ids', 'Прятать id за звёздочками', 'bool', False,
                hint='7095687 показывается как 70***87 — и в профиле у '
                     'человека, и в админских списках. Для съёмки экрана и '
                     'скриншотов в канал. В выгрузки (Excel) маска не идёт: '
                     'оттуда id копируют для начислений. Быстрее всего '
                     'переключать командой /mask'),
    )),
    ...
    Group('bypass', f'{e("bypass")} ByPass (белые списки)', (
        ...
        Setting('bypass.traffic_rate', 'Коэффициент списания трафика', 'float',
                1.0, min=0.01, max=10,
                hint='Сколько списывается с лимита за гигабайт, реально '
                     'ушедший в канал. 1.0 — один к одному; 0.1 — человек '
                     'качает десять, а платит за один. Значение здесь должно '
                     'совпадать с тем, что стоит у нод: бот по нему считает '
                     'настоящий расход в /bypassplan, сам он ничего не '
                     'списывает'),
        Setting('bypass.cost_month', 'Серверы ByPass в месяц', 'int', 0,
                unit='₽', min=0,
                hint='Сколько вы платите за серверы ByPass целиком за месяц. '
                     'Считается именно так, а не «за гигабайт»: серверы '
                     'оплачиваются помесячно независимо от прокачанного, и '
                     'себестоимость гигабайта из этого выводится сама — '
                     'в /bypassplan. Неделя × 4.33 = месяц'),
    ...
    Group('private', f'{e("private")} Личные серверы', (
        ...
        Setting('private.enabled', 'Приём заявок', 'bool', True,
                hint='Выключает покупку, не трогая уже работающие серверы. '
                     'Пока выключен, вместо витрины с тарифами человек видит '
                     'текст ниже — «Когда серверов нет». Переключается ещё '
                     'кнопкой в /queue'),
        Setting('private.autocharge', 'Продлевать серверы за деньги',
                'bool', True,
                hint='Выключено — плата с владельцев не берётся, а каждый '
                     'сервер дорабатывает оплаченный срок и закрывается: '
                     'доступ снимается, вам приходит «можно гасить VPS». '
                     'Оплаченное остаётся у человека целиком, за три дня до '
                     'конца ему уходит предупреждение. Так сворачивают '
                     'продукт, ничего не отнимая'),
        Setting('private.sold_out_note', 'Когда серверов нет', 'text',
                'Свободные серверы закончились: все машины, что у нас есть, '
                'уже заняты. Поднимем новые — продажа откроется снова, '
                'объявим об этом в канале.',
                hint='Показывается вместо тарифов, когда «Приём заявок» '
                     'выключен. Уже работающие серверы это не трогает: их '
                     'владельцы видят свой экран как обычно'),
        Setting('private.auto_give', 'Выдавать из запаса сразу', 'bool', True,
                hint='Если под заявку есть готовая машина из запаса '
                     '(/pooladd), сервер выдаётся в момент покупки — человек '
                     'не ждёт, а заявка к вам не приходит. Выключите, если '
                     'хотите смотреть каждую заявку глазами'),
        ...
        Setting('private.wait_bonus_days', 'Долгое ожидание — от скольких дней',
                'int', 3, unit=' дн.', min=0,
                hint='Заявку отменили, а человек ждал дольше этого срока — '
                     'вместе с деньгами он получает прибавку к следующему '
                     'пополнению. 0 — давать прибавку при любой отмене'),
        Setting('private.wait_bonus_rate', 'Прибавка за долгое ожидание',
                'percent', 0.15, min=0, max=1,
                hint='Разовая: начисляется один раз, к первому же пополнению '
                     'после отмены, сверх обычного бонуса. 0 — не давать'),
        Setting('private.sold_out_reason', 'Причина при отказе всем', 'text',
                'Свободные машины закончились, а новые в вашей локации мы '
                'пока поднять не можем.',
                hint='Уходит человеку как есть, когда вы жмёте «Отказать '
                     'всем» в очереди. Деньги и прибавка за ожидание '
                     'добавляются к этому тексту сами'),
    ...
    Group('raffle', f'{e("gift")} Розыгрыш', (
        Setting('raffle.start', 'Начало акции', 'str', '',
                hint='ДД.ММ.ГГГГ. Оплаты друзей считаются с 00:00 этого дня'),
        Setting('raffle.end', 'Конец акции', 'str', '',
                hint='ДД.ММ.ГГГГ включительно: оплата в 23:50 последнего дня '
                     'ещё в зачёте'),
        Setting('raffle.friend_tickets', 'Билетов за нового друга', 'int', 3,
                min=0,
                hint='Сколько билетов даёт приглашённый друг, купивший '
                     'подписку. Повторные покупки того же друга билетов не '
                     'дают — только новые люди'),
        Setting('raffle.self_per_month', 'Билетов за месяц своей подписки',
                'int', 1, min=0,
                hint='Купил полгода — шесть билетов. Это то, что превращает '
                     'акцию из «приведи друзей» в «продлись сейчас», а вторых '
                     'гораздо больше'),
        Setting('raffle.min_months', 'Минимальная подписка друга', 'int', 1,
                unit=' мес.', min=1,
                hint='Короче этого срока покупка друга билетов не даёт'),
        Setting('raffle.prizes', 'Призы', 'text',
                'iPhone 18 Pro\nAirPods 5\n5000₽ x10\nПодписка на месяц x25',
                hint='По одному в строке, одинаковые — с количеством через '
                     '«x»: «5000₽ x10». Победителей будет столько же, '
                     'сколько призов: жребий (/raffledraw) выдаёт каждому '
                     'его приз, а не номер строки. Писем победителям бот '
                     'не шлёт — только показывает список вам'),
        Setting('raffle.rules', 'Правила на экране', 'text',
                '3 билета — за друга, который придёт по вашей ссылке и купит '
                'подписку от месяца. Друг должен быть новым: за повторные '
                'покупки тех, кто уже был в боте, билеты не дают.\n'
                '1 билет — за каждый месяц вашей собственной подписки.\n'
                'Подписка друга должна быть активной на момент розыгрыша. '
                'Билетов может быть сколько угодно — чем больше, тем выше шанс.',
                hint='Короткий текст под числом билетов. Полные правила — в '
                     'посте канала, здесь только суть'),
        Setting('raffle.bonus_tickets', 'Подарок за сколько билетов', 'int', 0,
                min=0,
                hint='Утешительный подарок тем, кто не выиграет: набрал '
                     'столько билетов — получил дни, без всякого жребия. '
                     '0 — не дарить никому, и тогда экран про подарок молчит. '
                     'Считается в билетах, а не в друзьях: при цене три '
                     'билета за друга порог 3 — это один друг. Список тех, '
                     'кто дотянулся: /rafflebonus, начисляет их человек, '
                     'а не бот'),
        Setting('raffle.bonus_days', 'Сколько дней дарим', 'int', 7, unit=' дн.',
                min=0,
                hint='Размер утешительного подарка. Работает, только если '
                     'задан порог выше. Порог стоит держать достижимым: он '
                     'работает не как приз, а как цель — «остался один друг '
                     'до подарка»'),
        Setting('raffle.exclude', 'Кого не учитывать', 'str', '',
                hint='id через запятую: свои и рабочие аккаунты, аккаунты '
                     'команды. Исключённый не получает билетов сам и не '
                     'приносит их тому, кто его привёл. Правило честное, '
                     'но приз, ушедший внутрь, обесценивает всю акцию'),
        Setting('raffle.require_active', 'Подписка друга должна быть активной',
                'bool', True,
                hint='Проверяется на момент выгрузки. Без этого выгодно '
                     'оплатить минимум на десять аккаунтов, получить билеты и '
                     'всё бросить'),
    )),
    ...
    Group('lifeline', f'{e("knot")} Lifeline (сервер после истечения)', (
        ...
        Setting('lifeline.max_days', 'Дольше этого не держим', 'int', 14,
                unit=' дн.', min=0,
                hint='Запасной сервер нужен, чтобы человек успел открыть бота '
                     'и продлить. Пока предела не было, окно продлевалось на '
                     'каждом истечении — то есть бесплатный сервер навсегда '
                     'для всех, кто хоть раз платил. 0 — снова без предела. '
                     'Кто на нём сидит и сколько, видно в /freebies'),
    ...
    Group('maintenance', f'{e("wrench")} Технические работы', (
        Setting('features.maintenance_mode', 'Режим техработ', 'bool', False,
                hint='Пока включён, бот ничего не делает: на любое действие '
                     'человек видит один экран о работах. Подписки это не '
                     'трогает — VPN у людей продолжает работать. Админов '
                     'режим не касается, вы можете всё проверить'),
        Setting('text.maintenance', 'Что написать людям', 'text',
                'Обновляем бота. Совсем скоро вернёмся — обычно это занимает '
                'меньше получаса.',
                hint='Основной текст экрана. Про то, что подписка продолжает '
                     'работать, бот допишет сам'),
        Setting('features.maintenance_game', 'Сапёр на экране ожидания', 'bool', True,
                hint='Кнопка «сыграть, пока ждём». Ничего в боте не трогает '
                     'и работает, даже когда не работает всё остальное'),
        Setting('game.width', 'Ширина поля', 'int', 8, min=3, max=8,
                hint='Больше восьми Telegram в один ряд не ставит'),
        Setting('game.height', 'Высота поля', 'int', 8, min=3, max=10,
                hint='Выше десяти клавиатура не помещается на экран телефона '
                     'и упирается в предел Telegram в сто кнопок'),
        Setting('game.mines', 'Мин на поле', 'int', 10, min=1, max=79,
                hint='8×8 и десять мин — классический «новичок». Больше мин '
                     'на том же поле — сложнее и злее'),
    )),

    Group('posts', f'{e("channel")} Посты в канал', (
        Setting('post.channel', 'Канал для постов', 'str', '',
                hint='@юзернейм или числовой id. Пусто — берём канал из '
                     '«Бесплатный период»: он тот же самый. Бот должен быть '
                     'админом канала с правом публиковать сообщения'),
        Setting('post.button', 'Надпись на кнопке под постом', 'str',
                'Подключить RS VPN',
                hint='Кнопка ведёт в бота и добавляется к посту сама. '
                     'Надпись можно поменять и для одного поста — при '
                     'отправке. Новый пост: /post'),
    )),

    Group('partner', f'{e("friends")} Партнёрские боты', (
        Setting('partner.greeting', 'Что показывает бот партнёра', 'text',
                '<b>RS VPN</b> — быстрый VPN без рекламы и ограничений.\n\n'
                'Нажмите кнопку ниже, чтобы получить доступ. Первые дни — '
                'бесплатно.',
                hint='Текст в боте партнёра. Кнопка под ним ведёт в основного '
                     'бота с меткой партнёра, и приведённый засчитается ему'),
        Setting('partner.button', 'Надпись на кнопке', 'str',
                'Получить VPN'),
    )),
