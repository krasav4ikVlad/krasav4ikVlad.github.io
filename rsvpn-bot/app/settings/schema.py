...
SCHEMA: tuple[Group, ...] = (
    ...
    Group('raffle', f'{e("gift")} Розыгрыш', (
        ...
        Setting('raffle.rules', 'Правила на экране', 'text',
                'Билет даётся за друга, который пришёл по вашей ссылке, '
                'раньше в боте не был и оплатил подписку. Билетов может быть '
                'сколько угодно — чем больше, тем выше шанс.',
                hint='Короткий текст под числом билетов. Полные правила — в '
                     'посте канала, здесь только суть'),
        Setting('raffle.bonus_tickets', 'Подарок за сколько билетов', 'int', 3,
                min=0,
                hint='Кому подарок причитается без розыгрыша. 3 — привёл трёх '
                     'заплативших друзей и получил дни. 0 — не дарить никому. '
                     'Список тех, кто дотянулся: /rafflebonus'),
        Setting('raffle.bonus_days', 'Сколько дней дарим', 'int', 7, unit=' дн.',
                min=0,
                hint='Порог стоит держать достижимым: он работает не как '
                     'приз, а как цель — «остался один друг до подарка»'),
