...
SECTIONS: tuple[Section, ...] = (
    Section(f'{e("settings")} Управление', (
        ...
        Cmd('/panelids', 'Перевести подписки на числовые id панели 3.x',
            'Нужна один раз — сразу после обновления Remnawave до 3.x. '
            'С этой версии панель опознаёт подписку числовым id, а не uuid, '
            'и всё, что заведено раньше, она не понимает. Показ без правки; '
            'с аргументом <code>fix</code> переводит. Пока панель прежняя, '
            'команда отвечает, что переезжать некуда. То же делает фоновая '
            'задача раз в час'),
