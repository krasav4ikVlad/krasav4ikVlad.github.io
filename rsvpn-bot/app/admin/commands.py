...
SECTIONS: tuple[Section, ...] = (
    Section(f'{e("settings")} Управление', (
        ...
        Cmd('/bypasssync', 'У кого дата ByPass разошлась с подпиской',
            'Показ без правки. С аргументом <code>fix</code> выравнивает по '
            'основной подписке. То же самое делает фоновая задача раз в '
            'шесть часов — команда нужна, когда ждать некогда'),
    )),
    ...
    Section(f'{e("private")} Личные серверы', (
        ...
        Cmd('/pooladd UUID nl reality', 'Добавить поднятую машину в запас',
            'Заявка на такую машину выдаётся сама в момент покупки — человек '
            'получает сервер сразу. Если заявка уже ждала, она выдастся '
            'в момент добавления'),
