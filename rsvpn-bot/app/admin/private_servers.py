...
async def queue_text(c, settings=None) -> str:
    ...
    if settings is not None and not await settings.flag('private.autocharge'):
        lines.append(f'{e("warning")} <b>Продление выключено.</b> '
                     f'Ежемесячная плата не берётся; каждый сервер '
                     f'дорабатывает оплаченное и закрывается сам. Владельцу '
                     f'уходит предупреждение за 3 дня, вам — сообщение '
                     f'«можно гасить VPS».')
        lines.append('')
