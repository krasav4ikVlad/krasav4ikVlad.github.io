...
#
# Перевода подписок на числовые id панели здесь нет: он идёт по всей базе,
# занимает минуты и к обновлению бота отношения не имеет. Его место — день,
# когда обновляют саму панель:
#
#     python -m scripts.panel_ids        # посмотреть, ничего не меняя
