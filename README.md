...
**VPN Checker** (`checker.nodewiki.info`, отдельный РУ-сервер) — `checker_app.py`:
вставляешь VLESS-ссылку или JSON-конфиг (содержимое), сервис вытаскивает все
`host:port` и проверяет по каждому ICMP / TCP / HTTP GET / POST. Токены —
token-bucket на пользователя (1 токен за эндпоинт, дозаправка со временем),
тесты идут через очередь с пулом воркеров (наплыв пользователей обрабатывается
по очереди). SSRF-защита: служебные/приватные диапазоны не проверяются. Вход
общий (SSO). Деплой: `deploy-checker.sh` с `TOKEN_DB` и тем же `SECRET_KEY`,
что на основном сервере.
