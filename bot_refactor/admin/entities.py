...
from loader import promo_codes, support_quick_replies
from core.plans import invalidate_plans, plans_col
...
PLANS_ADMIN = EntityAdmin(
    ...
    fields=(
        ...
        Setting('gift_type', 'Тип подарка', 'str', '',
                hint='1day / 1month, пусто — без подарка'),
    ...
    on_change=invalidate_plans,
)

QUICK_REPLIES_ADMIN = EntityAdmin(
    code='qr',
    title='🧩 Быстрые ответы поддержки',
    ...
    label=lambda x: x.get('title', '?'),
    toggle_field='active',
    id_hint='Отправьте ключ ответа (например: how_to_connect).',
)

PROMO_ADMIN = EntityAdmin(
    code='promo',
    title='🎁 Промокоды',
    collection=promo_codes,
    id_field='code',
    fields=(
        Setting('reward_type', 'Тип награды', 'str', 'balance',
                hint='balance или traffic_gb'),
...
ENTITIES: dict[str, EntityAdmin] = {
    e.code: e for e in (PLANS_ADMIN, QUICK_REPLIES_ADMIN, PROMO_ADMIN)
}
