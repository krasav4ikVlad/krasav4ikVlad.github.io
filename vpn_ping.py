...
def print_table(results: list[CheckResult], color: bool = True) -> None:
    ...
    for r in sorted(results, key=lambda r: (r.status != "online", r.tcp_ms or 9e9)):
        hp = f"{r.host}:{r.port}"
        line = (
...
def main(argv: list[str]) -> int:
    ...
    servers = parse_servers(decoded)
    if not servers:
        print("В подписке не нашлось ни одного сервера.", file=sys.stderr)
        return 3

    print(f"Найдено серверов: {len(servers)}. Проверяю...", file=sys.stderr)
