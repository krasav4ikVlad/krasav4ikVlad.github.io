      ...
      (a.norm_mode === 'manual'
        ...
        : a.norm_mode === 'team_avg'
          ? `Норма — средний темп команды за период` +
            ...
            `, × часы оператора. `
          : `Норма — по нагрузке: за период было ${(data.demand || {}).waits ?? '—'} обращений и ` +
            `${(data.demand || {}).tickets ?? '—'} тикетов = ${(data.demand || {}).potential_points ?? '—'} доступных баллов, ` +
            `они распределяются по часам операторов` +
      ...
      <form id="act-set">
        <div class="field"><label>Норма баллов в час</label>
          ...
          <div class="muted" style="font-size:12px; margin-top:4px">
            <b>По нагрузке</b>: норма — это баллы, которые «лежали» во входящем потоке
            (5 за каждый ждавший ответа вопрос + 10 за каждый тикет), поделённые между
            операторами пропорционально их часам. Если все молчат — поток не исчезает
            и коэффициенты падают у всех; если тикетов не было — коэффициент 1.0.
            Честно работает даже с двумя операторами.<br>
