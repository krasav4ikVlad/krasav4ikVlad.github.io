...
добавить системную запись С ЛОГИНОМ закрывшего — тогда закрытие видно в переписке
на сайте и в справочной колонке «Закрыто» (на баллы и норму закрытия не влияют;
авто-закрытия — без логина):
...
* системная запись «Тикет закрыт (авто)…» пишется без `operator_login`;
  закрытия тикетов вообще не входят в баллы и норму «Активности» —
  оцениваются только ответы, их скорость и оценки пользователей.
